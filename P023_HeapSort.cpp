...
// Descripción: Ordenamiento Heap Sort con menú interactivo y gestión de archivos
...
// --- Funciones de Algoritmo Heap Sort ---

// Función auxiliar para mantener la propiedad del max-heap
void heapify(int arr[], int n, int i) {
    int mayor = i; // Inicialmente el nodo actual es el mayor
    int izquierda = 2 * i + 1; // Hijo izquierdo
    int derecha = 2 * i + 2;   // Hijo derecho

    // Comparar hijo izquierdo con el nodo actual
    ...
    // Si el mayor no es el nodo raíz, intercambiar y aplicar recursión
    if (mayor != i) {
...
// Función principal de Heap Sort[cite: 6]
void heapSort(int arr[], int n) {
    // 1. Construcción del max-heap inicial[cite: 6]
    for (int i = n / 2 - 1; i >= 0; i--) {
        heapify(arr, n, i);
    }

    // 2. Extracción uno por uno de los elementos del heap[cite: 6]
    for (int i = n - 1; i > 0; i--) {
        // Mover la raíz (el mayor) al final del arreglo actual
        swap(arr[0], arr[i]);
        // Re-ajustar el heap en el área reducida[cite: 6]
        heapify(arr, i, 0);
    }
}

// --- Funciones de Gestión de Archivos y Menú ---
...
void guardarArchivo() {
    ...
    string nombre;
    cout << "Ingrese el nombre para el archivo ordenado (sin extensión): ";
    cin >> ws;
    getline(cin, nombre);
    nombre += ".txt";
    ...
    archivo.close();
    archivoGrabado = true;
    cout << "Archivo grabado exitosamente.\n";
}
...
int main() {
    setlocale(LC_ALL, ""); // Soporte para caracteres en español[cite: 6]
    int opcion;
    bool continuar = true;
